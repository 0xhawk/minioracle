import { Client, registry, MissingWalletError } from 'minioracle-client-ts'

import { Comment } from "minioracle-client-ts/minioracle.minioracle/types"
import { Params } from "minioracle-client-ts/minioracle.minioracle/types"
import { Post } from "minioracle-client-ts/minioracle.minioracle/types"
...
export default {
	...
	getters: {
			...
			return state.Params[JSON.stringify(params)] ?? {}
				...
				getSay: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Say[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: minioracle.minioracle initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.MinioracleMinioracle.query.queryParams()).data
		...
		async QuerySay({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.MinioracleMinioracle.query.querySay()).data
				
					
				commit('QUERY', { query: 'Say', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QuerySay', payload: { options: { all }, params: {...key},query }})
				return getters['getSay']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QuerySay API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryPosts({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.MinioracleMinioracle.query.queryPosts(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.MinioracleMinioracle.query.queryPosts({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async QueryComment({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.MinioracleMinioracle.query.queryComment( key.id)).data
		...
		async QueryCommentAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.MinioracleMinioracle.query.queryCommentAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.MinioracleMinioracle.query.queryCommentAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
					value = mergeResults(value, next_values);
				}
		...
		async sendMsgCreatePost({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.MinioracleMinioracle.tx.sendMsgCreatePost({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgCreatePost({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.MinioracleMinioracle.tx.msgCreatePost({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreatePost:Init Could not initialize signing client. Wallet is required.')
