...
import { MsgCreatePost } from "./types/minioracle/minioracle/tx";
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgCreatePost({ value }: msgCreatePostParams): EncodeObject {
			try {
				return { typeUrl: "/minioracle.minioracle.MsgCreatePost", value: MsgCreatePost.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			MinioracleMinioracle: new SDKModule(test)
		},
		registry: msgTypes
  }
}
